...
void winsock_server::rec_mode(int socket_number) {
	...
	while (true) {
		...
		buffer_full = true;
		std::string s = "From user " + std::to_string(socket_number) + ": " + message_buffer;
		Print(s);

		if (strcmp(message_buffer, "quit") == 0) {
			s = "Closing connection with " + socket_number;
			Print(s);
			closesocket(this->connection_socket[socket_number]);
			this->active_sockets[socket_number] = 0;
			break;
...
void winsock_server::send_mode() {
	...
	while (true) {
		if (buffer_full) {
			...
			if (strcmp(message_buffer, "quit") == 0) {
				std::string s = "Closing connection with " + message_sender;
				Print(s);
